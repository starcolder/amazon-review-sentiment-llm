...
enter the number you want to choose(1/2): """)
    if answer == "1":
        model = "myqwen"
        backup_path = os.path.join(BASE_DIR, "backups", "backup_qwen.csv")
        final_path = os.path.join(BASE_DIR, "results", "final_qwen.csv")
    elif answer == "2":
        model = "myphi"
        backup_path = os.path.join(BASE_DIR, "backups", "backup_phi.csv")
        final_path = os.path.join(BASE_DIR, "results", "final_phi.csv")
...
for i in range(start_idx, len(dev_df)):
    text = str(dev_df.iloc[i]["text"])[:300]
...
Sentiment:"""
    ...
    speed = (i + 1) / elapsed
    print(
        f"\rprocessed: {i+1} out of {len(dev_df)} | speed: {speed:.2f} records/sec",
        end="",
        flush=True
    )
